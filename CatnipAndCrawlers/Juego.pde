class Juego {
  ...
  void mousePresionado(float mx, float my) {
    ...
    if (my >= menuY && my <= menuY + slotAlto) {
      ...
      if (mx >= x1 && mx <= x1 + slotAncho) {
        ...
        colorSemilla = color(100, 255, 100);
      }
  ...
  void mouseSoltado(float mx, float my) {
    if (arrastrandoSemilla) {
      jardin.intentarPlantar(mx, my, semillaSeleccionada, colorSemilla); 
